...
- [x] 阻断：账号密码使用 PBKDF2-SHA-256、随机盐和版本化参数保存校验值。
- [x] 阻断：Repository 与 File Adapter 强制当前账号隔离。
- [x] 阻断：单据、库存流水和快照通过同一 IndexedDB transaction 提交。
- [x] 阻断：编号具有账号内复合唯一索引约束。
- [x] 合同附件 Blob 与元数据分离，并通过 File Adapter 访问。
- [x] 页面层和领域层不直接调用 IndexedDB 或附件 Blob store。
- [x] schema 版本化，升级失败不删除旧数据库且阻止继续写入。
...
- 2026-06-12，任务 38 已实现 IndexedDB + Web Crypto Auth；6 项专项测试覆盖 PBKDF2 随机盐、旧明文迁移与失败回滚、重复用户名、会话恢复和双账号独立凭据，完整 69 项测试与核心流程 E2E 通过。
- 2026-06-12，任务 39 已实现 IndexedDB 业务 Repository、草稿和迁移器；进货/出货单据与库存同 transaction 提交，账号内复合唯一索引生效，双账号、故障注入、幂等迁移、损坏源数据和版本过新测试通过。
- 任务 39 完整自动化为 9 个测试文件、74 项全部通过，生产构建和核心流程 E2E 通过。
- 2026-06-15，任务 40 已实现账号隔离 File Adapter、合同附件三 store 事务和无孤儿删除；8 项附件专项测试及真实上传下载 E2E 通过。
- 2026-06-15，任务 43 真实浏览器双账号迁移演练验证账号、凭据、Repository、合同与附件隔离；四类单据并发编号和空库 schema 升级专项回归通过。
- 未登录的旧账号仍需在首次成功输入原密码后删除历史明文，日志脱敏和生产候选环境验证仍未完成，因此 Gate 3 保持未通过。

## Gate 4：本地迁移与数据完整性

- [x] 阻断：localStorage 到 IndexedDB 的迁移有版本号、幂等标记和完整性校验。
- [x] 阻断：迁移失败保留旧数据，不留下半迁移状态。
- [ ] 阻断：迁移前后实体数量、金额、库存和附件核对一致。
- [x] 明文密码只在成功登录后迁移，成功后删除对应明文。
- [x] Base64 附件迁移到 Blob 后仍可下载，失败时保留旧附件。
- [x] schema 升级兼容已有版本，版本过新时拒绝由旧程序写入。
- [ ] 已使用脱敏多账号样本完成至少两次迁移演练。

当前证据（尚不足以通过 Gate）：
- 2026-06-12，任务 39 的迁移专项测试覆盖双账号业务数据与草稿、重复执行、旧数据保留、唯一索引冲突、损坏 JSON 和数据库版本过新。
- 结构化数据迁移已自动化通过。
- 2026-06-15，任务 40 已验证 Base64 附件迁移后的文件内容、元数据、数量和总字节数，损坏附件会整体回滚并保留来源。
- 2026-06-15，任务 43 使用两套独立脱敏双账号样本完成真实 Chromium 迁移演练；迁移前后账号、业务实体、三类单据金额、库存、附件数量与字节摘要一致，旧数据保留且明文密码完成登录迁移后删除。
- 自动化迁移证据已完成；目标 Windows 恢复和人工核对已补证通过。Gate 4 仍保留未通过，原因是本次手动记录未提供可追溯的逐项迁移前后数量、金额、库存和附件摘要明细。
...
- 2026-06-15，任务 43 已验证 E2E 清理不会残留模板缓存，模板或动态导出组件未缓存时会提示联网重试。
...
- [x] 阻断：`.tradgio-backup` 具有格式版本、schema 版本、完整性摘要和记录统计。
- [x] 阻断：备份覆盖全部账号、业务数据和附件，不包含活动 session。
- [x] 阻断：备份压缩后使用 AES-256-GCM 加密，密码和派生密钥不落盘。
- [x] 阻断：错误密码、篡改、版本不兼容或容量不足时不修改现有数据。
- [x] 阻断：已在全新 Windows 浏览器配置中恢复备份并通过核心业务校验。
- [x] 恢复前自动生成快照，恢复失败可安全回到原数据。
- [x] Storage API 展示容量；70% 提醒、85% 阻止高容量写入。
- [x] 备份提醒、至少两处副本和恢复演练频率已记录。
- [x] 用户已被明确告知忘记备份密码无法恢复。
...
- 2026-06-15，任务 42 已实现真实 gzip、PBKDF2 600,000 次派生、AES-256-GCM 加解密、15 store 与附件完整性校验、容量预检、恢复预览、恢复前快照、单 transaction 整库替换、恢复后摘要复核和恢复报告。
- 4 项专项测试和真实 Chromium E2E 验证错误密码、篡改、容量阻断、快照下载失败、完整恢复与 session 清除；完整质量门禁 12 个测试文件、94 项测试通过，2 条 E2E 通过。
- 2026-06-16，任务 45 已实现“数据备份”页备份提醒、上次备份后变更量和 Storage API 存储健康面板；成功保存备份后将上次备份时间和摘要写入 `appSettings`，不记录密码、派生密钥或附件内容。专项测试覆盖无备份、备份后无变更、备份后变更、70% 提醒和 85% 阻断状态；浏览器验证页面渲染和表单交互通过。
- 2026-06-16，已新增 `docs/windows-recovery-rehearsal.md`，用于目标 Windows Chrome/Edge 全新配置恢复演练和人工核对记录；该文档仅代表执行包已准备，不代表实机恢复已完成。
- 2026-06-16，目标 Windows Chrome `149.0.7827.115` 与 Edge `149.0.4022.69` 已使用全新配置路径分别完成 `.tradgio-backup` 恢复；恢复后账号、货品、往来单位、进货、出货、报价、合同、附件、库存、搜索和导出均由用户人工核对通过。应用版本按当前发布版本 `0.1.0` 记录，IndexedDB schema 按当前发布版本 `1` 记录；用户原始手动记录曾填 `2`，已标记为待复核的手填差异。

## Gate 7：UAT 与发布结论

- [x] 阻断：真实业务样本完成货品、单位、进货、出货、报价和合同 UAT。
