...
页面与 IndexedDB、Web Crypto、Storage API、Service Worker 之间的统一 Adapter、运行配置、错误映射、测试替身和 PWA 更新状态机见 `specs/platform-adapters-pwa.md`。Service Worker 只提示 waiting 更新，禁止自动刷新编辑页面。
...
合同保存前通过 Storage API 估算追加附件后的使用率：达到 70% 提醒备份和清理，达到 85% 阻止写入；浏览器无法可靠估算时仍执行 20 MB 单文件限制并显示容量未知提醒。合同、附件元数据和 Blob 使用同一个 IndexedDB transaction，删除附件或合同同步删除二进制，不留孤儿记录。
