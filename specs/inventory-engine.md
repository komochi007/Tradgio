...
- 快照只是聚合结果，不是唯一事实来源
- 流水和快照必须包含 `accountId`
- 库存读取、历史、预警和更新只能作用于当前账号
- 库存输入中的账号必须与当前 Local Auth session 一致
...
- 先按货品聚合新旧单据中的重复明细
- 原始数量差额为 `nextQuantity - previousQuantity`
- 进货库存差额为原始数量差额，出货库存差额为原始数量差额乘以 `-1`
- 数量增加、数量减少、增加明细、删除明细和替换货品都必须保留正负方向
- 流水中的 `quantityDelta` 记录实际库存方向，`balanceAfter` 基于当前快照顺序计算

示例：
- 进货数量 `8 -> 5`，库存差额为 `-3`
- 出货数量 `8 -> 5`，库存差额为 `+3`
- 进货货品 `A -> B` 且数量均为 `4`，库存差额为 `A: -4`、`B: +4`
- 出货货品 `A -> B` 且数量均为 `4`，库存差额为 `A: +4`、`B: -4`
