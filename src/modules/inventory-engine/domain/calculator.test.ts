...
function computeRecalcEntries(
  ...
  return computeLedgerEntries(deltaOrder!, currentSnapshots);
}
...
describe("库存改单差额计算", () => {
  ...
  it.fails("已知缺陷：替换进货货品应撤销旧货品并增加新货品", () => {
    const entries = computeRecalcEntries(
  ...
  it.fails("已知缺陷：替换出货货品应恢复旧货品并扣减新货品", () => {
    const entries = computeRecalcEntries(
