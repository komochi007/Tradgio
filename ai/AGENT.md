...
- 生产化后续工作当前从任务 44 开始，以生产化路线图为执行依据
...
生产化任务额外规则：
- 按路线图依赖顺序推进，当前下一任务为任务 44
- 任务 34 已由 ADR-0002 锁定 IndexedDB 本地优先方案，不得擅自恢复云端后端路线
- 任务 35-43 已完成；任务 44 完成前不进入 Windows 正式上线验收
...
- 当前认证、结构化业务数据和附件 Blob 已切换 IndexedDB，客户端导出支持离线缓存，整机加密备份恢复已实现；PWA 正式发布仍未完成
- 未登录旧账号的明文迁移、Windows 恢复演练和 PWA 发布是当前剩余高优先级风险
