...
- 生产化后续工作当前从任务 42 开始，以生产化路线图为执行依据
...
生产化任务额外规则：
- 按路线图依赖顺序推进，当前下一任务为任务 42
- 任务 34 已由 ADR-0002 锁定 IndexedDB 本地优先方案，不得擅自恢复云端后端路线
- 任务 35-41 已完成；任务 42 完成前不进入 PWA 正式发布
...
- 当前认证、结构化业务数据和附件 Blob 已切换 IndexedDB，客户端导出已支持按需加载和模板离线缓存；加密备份和 PWA 正式发布仍未完成
- 未登录旧账号的明文迁移和加密备份是当前优先级最高的风险
