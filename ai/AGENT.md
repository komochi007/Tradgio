...
生产化任务额外规则：
- 按路线图依赖顺序推进，当前从任务 26 开始
- P0 的任务 31 未通过前，不实施真实后端、对象存储或部署平台接入
- 平台供应商必须由任务 34 的 ADR 锁定，不根据个人偏好提前引入 SDK
...
- 当前认证、业务数据和附件仍是本地适配，不能按生产能力宣称完成
- 库存回算、写入一致性、账号隔离和编号加固是当前优先级最高的风险
