...
生产化任务额外规则：
- 按路线图依赖顺序推进，当前下一任务为任务 39
- 任务 34 已由 ADR-0002 锁定 IndexedDB 本地优先方案，不得擅自恢复云端后端路线
- 任务 35-38 已完成；任务 39-42 按依赖顺序实施，完成前不进入 PWA 正式发布
...
- 当前认证已切换 IndexedDB + Web Crypto；业务数据和附件仍是 localStorage / Base64 适配，不能按完整 IndexedDB 生产能力宣称完成
- IndexedDB 业务事务、未登录旧账号的明文迁移、附件 Blob 和加密备份是当前优先级最高的风险
